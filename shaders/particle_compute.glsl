...
layout(std140, binding = 0) uniform ParticleUniforms {
    ...
    vec3 hiddenPosition;
    float actualAppliedFrequency; // Добавляем частоту для адаптивного демпфирования
};
...
// Функция для безопасного получения значения из массива смещений
...
// Функция для получения смещения из поля FDM
float getDisplacementFromField(vec2 position) {
    ...
    return mix(mix(d00, d01, fracC), mix(d10, d11, fracC), fracR);
}

// Функция для получения градиента из поля FDM
...
void main() {
    ...
    vec2 force = -2.0 * disp * grad * forceMult;
